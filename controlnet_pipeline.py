...
class ControlnetCogVideoXImageToVideoPCDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __call__(
        ...
        anchor_videos: None,
        anchor_masks: None,
        gt_cameras: Optional[torch.Tensor] = None,
        context_cameras: Optional[torch.Tensor] = None,
        ...
        anchor_states = self.encode_video(anchor_videos.permute(0, 2, 1, 3, 4)).to(device)
        
        # Prepare cameras
        if gt_cameras is not None:
            gt_cameras = gt_cameras.to(device=device, dtype=prompt_embeds.dtype)
        if context_cameras is not None:
            context_cameras = context_cameras.to(device=device, dtype=prompt_embeds.dtype)
        ...
        if do_classifier_free_guidance:
            anchor_states = torch.stack([anchor_states] * 2)
            anchor_masks = torch.stack([anchor_masks] * 2)
            if gt_cameras is not None:
                gt_cameras = torch.stack([gt_cameras] * 2)
            if context_cameras is not None:
                context_cameras = torch.stack([context_cameras] * 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if (controlnet_guidance_start <= current_sampling_percent <= controlnet_guidance_end):
                    ...
                    # controlnet_output is (controlnet_hidden_states, camera_states_list)
                    ...
                    if isinstance(camera_states_list, (tuple, list)):
                        camera_states_list = [x.to(dtype=self.transformer.dtype) if x is not None else None for x in camera_states_list]
                    elif camera_states_list is not None:
                        camera_states_list = camera_states_list.to(dtype=self.transformer.dtype)
